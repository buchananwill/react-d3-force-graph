...
export function getModulusGridY<T extends HasNumberId>(
    ...
    if (typeof strength === 'number') {
        staticStrength = strength === 0 ? 0 : Math.pow(strength, exponentForPositionalForcesToCreateCurvedDelta)
    }

    return d3
        .forceY((_d: DataNode<T>, i) => {
            const distanceFromRoot = _d.distanceFromRoot;
            if (distanceFromRoot === undefined || isNaN(distanceFromRoot)) return height / 2;
...
export function updateForceY<T extends HasNumberId>(
    ...
    function consumer(forceY: d3.ForceY<DataNode<T>>) {
        const strength = forceYStrength === 0 ? 0 : Math.pow(forceYStrength, exponentForPositionalForcesToCreateCurvedDelta);
